...
public class CollectionServiceIntegrationTests(IntegrationFixture fixture)
{
    private CollectionService Service(IServiceScope scope) =>
    ...
    private Task<bool> CanView(IServiceScope scope, UserId user, CollectionId collection) =>
        Authz(scope).CheckAsync(
            $"{FgaTypes.User}:{user}",
            FgaRelations.CanView,
            $"{FgaTypes.Collection}:{collection}");

    private Task<bool> CanManage(IServiceScope scope, UserId user, CollectionId collection) =>
        Authz(scope).CheckAsync(
            $"{FgaTypes.User}:{user}",
    ...
    public async Task Create_OwnerCanManage()
    {
        ...
        // Owner's tuple was written — management operations succeed
    ...
    public async Task Unshare_RevokesViewAccess()
    {
        ...
        await Service(scope).CreateAsync(id, "Test", owner);
        await Service(scope).ShareWithUserAsync(id, owner, viewer);
        Assert.True(await CanView(scope, viewer, id)); // confirm share worked
    ...
    public async Task MakePrivate_RemovesPublicViewAccess()
    {
        ...
        await Service(scope).CreateAsync(id, "Test", owner);
        await Service(scope).MakePublicAsync(id, owner);
        Assert.True(await CanView(scope, anyone, id)); // confirm public worked
